...
$.Enemy.prototype.update = function(i) {
    ...
    if (
        this.lockBounds &&
        !$.util.arcInRect(this.x, this.y, this.radius + 10, 0, 0, $.ww, $.wh)
    ) {
        $.enemies.splice(i, 1);
    }
...
$.Enemy.prototype.renderHealth = function(i) {
    if (this.inView && this.life > 0 && this.life < this.lifeMax) {
        ...
        $.ctxmg.fillStyle =
            "hsla(" + (this.life / this.lifeMax) * 120 + ", 100%, 50%, 0.75)";
        $.ctxmg.fillRect(
            this.x - this.radius,
            this.y - this.radius - 6,
            this.radius * 2 * (this.life / this.lifeMax),
            3
        );
    }
};
...
$.Enemy.prototype.render = function(i) {
    if (this.inView) {
        ...
        $.util.strokeCircle(
            ...
            this.x,
            this.y,
            this.radius / 4 + (Math.cos(mod) * this.radius) / 4,
            this.strokeStyle,
            1.5
        );
        $.util.strokeCircle(
